...
public:
   ...
   SparseMatrix addMatrix(const SparseMatrix& right) {
      SparseMatrix result(*this);

      for (uint16_t i = 0; i < result.Size(); i++) {
         result.di[i] += right.di[i];
      }
      for (uint32_t i = 0; i < result.ggl.size(); i++) {
         result.ggl[i] += right.ggl[i];
         result.ggu[i] += right.ggu[i];
      }
      return result;
   }
   ...
   /// </summary>
   SparseMatrix multToScalar(const double scalar) const;
   SparseMatrix operator*(const double scalar) const {
      return this->multToScalar(scalar);
   }
...
public:
   ...
   static SparseMatrix CopyShape(const SparseMatrix& right) {
      SparseMatrix newMat;
      newMat.ig = right.ig;
      newMat.jg = right.jg;
      newMat.di.resize(right.di.size());
      newMat.ggl.resize(right.ggl.size());
      newMat.ggu.resize(right.ggu.size());
