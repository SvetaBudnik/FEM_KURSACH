...
class OneDimentionSolver {
   ...
   public:
    ...
    double compute() { return computeWithRange(from, to); }

    /// <summary>
    /// Function to compute quadrature with other range with step. Doesn't override previous ranges
    /// </summary>
    double computeWithStep(double from, double step) { return computeWithRange(from, from + step); }
};
...
class TwoDimentionalSolver {
   ...
   public:
    TwoDimentionalSolver(double xFrom, double xTo, double yFrom, double yTo,
                         std::function<double(double, double)> computeFunc)
        : xFrom(xFrom), xTo(xTo), yFrom(yFrom), yTo(yTo), computeFunc(computeFunc) {}

    static TwoDimentionalSolver withStep(double xFrom, double xStep, double yFrom, double yStep,
                                         std::function<double(double, double)> computeFunc) {
   ...
   public:
    ...
    /// </summary>
    /// <returns>Result of computation</returns>
    double compute() { return computeWithRange(xFrom, xTo, yFrom, yTo); }
